// Generated by Scapix Java API 2.1
// https://www.scapix.com

#include <scapix/java_api/java/util/TimeZone.h>

#ifndef SCAPIX_JAVA_API_SUN_UTIL_CALENDAR_ZONEINFO_FWD
#define SCAPIX_JAVA_API_SUN_UTIL_CALENDAR_ZONEINFO_FWD

#include <scapix/jni/java_api/macros_push.h>

namespace scapix::java_api { namespace sun::util::calendar { class ZoneInfo; }}

template<>
struct scapix::jni::object_traits<scapix::java_api::sun::util::calendar::ZoneInfo>
{
	static constexpr fixed_string class_name = "sun/util/calendar/ZoneInfo";
	using base_classes = std::tuple<scapix::java_api::java::util::TimeZone>;
};

#include <scapix/jni/java_api/macros_pop.h>

#endif // SCAPIX_JAVA_API_SUN_UTIL_CALENDAR_ZONEINFO_FWD

#if !defined(SCAPIX_JAVA_API_FWD) && !defined(SCAPIX_JAVA_API_SUN_UTIL_CALENDAR_ZONEINFO)
#define SCAPIX_JAVA_API_SUN_UTIL_CALENDAR_ZONEINFO

#define SCAPIX_JAVA_API_FWD
#include <scapix/java_api/java/lang/Object.h>
#include <scapix/java_api/java/lang/String.h>
#include <scapix/java_api/java/util/Date.h>
#include <scapix/java_api/java/util/Map.h>
#include <scapix/java_api/java/util/SimpleTimeZone.h>
#include <scapix/java_api/java/util/stream/Stream.h>
#undef SCAPIX_JAVA_API_FWD

#include <scapix/jni/java_api/macros_push.h>

namespace scapix::java_api {

class sun::util::calendar::ZoneInfo : public jni::object_base<"sun/util/calendar/ZoneInfo",
	java::util::TimeZone>
{
public:

	static jni::ref<sun::util::calendar::ZoneInfo> new_object() { return object_base::new_object(); }
	static jni::ref<sun::util::calendar::ZoneInfo> new_object(jni::ref<java::lang::String> ID, jint rawOffset) { return object_base::new_object(ID, rawOffset); }
	jint getOffset(jlong date) { return call_method<"getOffset", jint>(date); }
	jint getOffsets(jlong utc, jni::ref<jni::array<jint>> offsets) { return call_method<"getOffsets", jint>(utc, offsets); }
	jint getOffsetsByStandard(jlong standard, jni::ref<jni::array<jint>> offsets) { return call_method<"getOffsetsByStandard", jint>(standard, offsets); }
	jint getOffsetsByWall(jlong wall, jni::ref<jni::array<jint>> offsets) { return call_method<"getOffsetsByWall", jint>(wall, offsets); }
	jint getOffset(jint era, jint year, jint month, jint day, jint dayOfWeek, jint milliseconds) { return call_method<"getOffset", jint>(era, year, month, day, dayOfWeek, milliseconds); }
	void setRawOffset(jint offsetMillis) { return call_method<"setRawOffset", void>(offsetMillis); }
	jint getRawOffset() { return call_method<"getRawOffset", jint>(); }
	jboolean isDirty() { return call_method<"isDirty", jboolean>(); }
	jboolean useDaylightTime() { return call_method<"useDaylightTime", jboolean>(); }
	jboolean observesDaylightTime() { return call_method<"observesDaylightTime", jboolean>(); }
	jboolean inDaylightTime(jni::ref<java::util::Date> date) { return call_method<"inDaylightTime", jboolean>(date); }
	jint getDSTSavings() { return call_method<"getDSTSavings", jint>(); }
	jni::ref<java::lang::String> toString() { return call_method<"toString", jni::ref<java::lang::String>>(); }
	static jni::ref<jni::array<java::lang::String>> getAvailableIDs() { return call_static_method<"getAvailableIDs", jni::ref<jni::array<java::lang::String>>>(); }
	static jni::ref<jni::array<java::lang::String>> getAvailableIDs(jint rawOffset) { return call_static_method<"getAvailableIDs", jni::ref<jni::array<java::lang::String>>>(rawOffset); }
	static jni::ref<java::util::stream::Stream> availableIDs() { return call_static_method<"availableIDs", jni::ref<java::util::stream::Stream>>(); }
	static jni::ref<java::util::stream::Stream> availableIDs(jint rawOffset) { return call_static_method<"availableIDs", jni::ref<java::util::stream::Stream>>(rawOffset); }
	static jni::ref<java::util::TimeZone> getTimeZone(jni::ref<java::lang::String> ID) { return call_static_method<"getTimeZone", jni::ref<java::util::TimeZone>>(ID); }
	jni::ref<java::util::SimpleTimeZone> getLastRuleInstance() { return call_method<"getLastRuleInstance", jni::ref<java::util::SimpleTimeZone>>(); }
	jni::ref<java::lang::Object> clone() { return call_method<"clone", jni::ref<java::lang::Object>>(); }
	jint hashCode() { return call_method<"hashCode", jint>(); }
	jboolean equals(jni::ref<java::lang::Object> obj) { return call_method<"equals", jboolean>(obj); }
	jboolean hasSameRules(jni::ref<java::util::TimeZone> other) { return call_method<"hasSameRules", jboolean>(other); }
	static jni::ref<java::util::Map> getAliasTable() { return call_static_method<"getAliasTable", jni::ref<java::util::Map>>(); }

};

} // namespace scapix::java_api

#include <scapix/jni/java_api/macros_pop.h>

#endif // SCAPIX_JAVA_API_SUN_UTIL_CALENDAR_ZONEINFO
