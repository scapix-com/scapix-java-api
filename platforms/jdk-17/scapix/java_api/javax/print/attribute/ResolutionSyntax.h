// Generated by Scapix Java API 2.3
// https://www.scapix.com

#include <scapix/java_api/java/lang/Object.h>
#include <scapix/java_api/java/io/Serializable.h>
#include <scapix/java_api/java/lang/Cloneable.h>

#ifndef SCAPIX_JAVA_API_JAVAX_PRINT_ATTRIBUTE_RESOLUTIONSYNTAX_FWD
#define SCAPIX_JAVA_API_JAVAX_PRINT_ATTRIBUTE_RESOLUTIONSYNTAX_FWD

#include <scapix/jni/java_api/macros_push.h>

namespace scapix::java_api {
namespace javax::print::attribute {

class ResolutionSyntax;

} // namespace javax::print::attribute
} // namespace scapix::java_api

template<>
struct scapix::jni::object_traits<scapix::java_api::javax::print::attribute::ResolutionSyntax>
{
	static constexpr fixed_string class_name = "javax/print/attribute/ResolutionSyntax";
	using base_classes = std::tuple<scapix::java_api::java::lang::Object, scapix::java_api::java::io::Serializable, scapix::java_api::java::lang::Cloneable>;
};

#include <scapix/jni/java_api/macros_pop.h>

#endif // SCAPIX_JAVA_API_JAVAX_PRINT_ATTRIBUTE_RESOLUTIONSYNTAX_FWD

#ifndef SCAPIX_JAVA_API_FWD
#ifndef SCAPIX_JAVA_API_JAVAX_PRINT_ATTRIBUTE_RESOLUTIONSYNTAX
#define SCAPIX_JAVA_API_JAVAX_PRINT_ATTRIBUTE_RESOLUTIONSYNTAX

#define SCAPIX_JAVA_API_FWD
#include <scapix/java_api/java/lang/String.h>
#undef SCAPIX_JAVA_API_FWD

#include <scapix/jni/java_api/macros_push.h>

namespace scapix::java_api {

class javax::print::attribute::ResolutionSyntax : public jni::object_base<"javax/print/attribute/ResolutionSyntax",
	java::lang::Object,
	java::io::Serializable,
	java::lang::Cloneable>
{
public:

	static jint DPI() { return get_static_field<"DPI", jint>(); }
	static jint DPCM() { return get_static_field<"DPCM", jint>(); }

	static jni::ref<javax::print::attribute::ResolutionSyntax> new_object(jint crossFeedResolution, jint feedResolution, jint units) { return object_base::new_object(crossFeedResolution, feedResolution, units); }
	jni::ref<jni::array<jint>> getResolution(jint units) { return call_method<"getResolution", jni::ref<jni::array<jint>>>(units); }
	jint getCrossFeedResolution(jint units) { return call_method<"getCrossFeedResolution", jint>(units); }
	jint getFeedResolution(jint units) { return call_method<"getFeedResolution", jint>(units); }
	jni::ref<java::lang::String> toString(jint units, jni::ref<java::lang::String> unitsName) { return call_method<"toString", jni::ref<java::lang::String>>(units, unitsName); }
	jboolean lessThanOrEquals(jni::ref<javax::print::attribute::ResolutionSyntax> other) { return call_method<"lessThanOrEquals", jboolean>(other); }
	jboolean equals(jni::ref<java::lang::Object> object) { return call_method<"equals", jboolean>(object); }
	jint hashCode() { return call_method<"hashCode", jint>(); }
	jni::ref<java::lang::String> toString() { return call_method<"toString", jni::ref<java::lang::String>>(); }
};

} // namespace scapix::java_api

#include <scapix/jni/java_api/macros_pop.h>

#endif // SCAPIX_JAVA_API_JAVAX_PRINT_ATTRIBUTE_RESOLUTIONSYNTAX
#endif // SCAPIX_JAVA_API_FWD
