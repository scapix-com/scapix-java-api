// Generated by Scapix Java API 2.1
// https://www.scapix.com

#include <scapix/java_api/java/lang/invoke/VarHandle.h>

#ifndef SCAPIX_JAVA_API_JAVA_LANG_INVOKE_INDIRECTVARHANDLE_FWD
#define SCAPIX_JAVA_API_JAVA_LANG_INVOKE_INDIRECTVARHANDLE_FWD

#include <scapix/jni/java_api/macros_push.h>

namespace scapix::java_api { namespace java::lang::invoke { class IndirectVarHandle; }}

template<>
struct scapix::jni::object_traits<scapix::java_api::java::lang::invoke::IndirectVarHandle>
{
	static constexpr fixed_string class_name = "java/lang/invoke/IndirectVarHandle";
	using base_classes = std::tuple<scapix::java_api::java::lang::invoke::VarHandle>;
};

#include <scapix/jni/java_api/macros_pop.h>

#endif // SCAPIX_JAVA_API_JAVA_LANG_INVOKE_INDIRECTVARHANDLE_FWD

#if !defined(SCAPIX_JAVA_API_FWD) && !defined(SCAPIX_JAVA_API_JAVA_LANG_INVOKE_INDIRECTVARHANDLE)
#define SCAPIX_JAVA_API_JAVA_LANG_INVOKE_INDIRECTVARHANDLE

#define SCAPIX_JAVA_API_FWD
#include <scapix/java_api/java/lang/Class.h>
#include <scapix/java_api/java/lang/invoke/MethodHandle.h>
#include <scapix/java_api/java/lang/invoke/VarHandle_AccessMode.h>
#include <scapix/java_api/java/util/List.h>
#undef SCAPIX_JAVA_API_FWD

#include <scapix/jni/java_api/macros_push.h>

namespace scapix::java_api {

// private
class java::lang::invoke::IndirectVarHandle : public jni::object_base<"java/lang/invoke/IndirectVarHandle",
	java::lang::invoke::VarHandle>
{
public:

	jni::ref<java::lang::Class> varType() { return call_method<"varType", jni::ref<java::lang::Class>>(); }
	jni::ref<java::util::List> coordinateTypes() { return call_method<"coordinateTypes", jni::ref<java::util::List>>(); }
	jni::ref<java::lang::invoke::VarHandle> withInvokeExactBehavior() { return call_method<"withInvokeExactBehavior", jni::ref<java::lang::invoke::VarHandle>>(); }
	jni::ref<java::lang::invoke::VarHandle> withInvokeBehavior() { return call_method<"withInvokeBehavior", jni::ref<java::lang::invoke::VarHandle>>(); }
	jni::ref<java::lang::invoke::MethodHandle> toMethodHandle(jni::ref<java::lang::invoke::VarHandle_AccessMode> accessMode) { return call_method<"toMethodHandle", jni::ref<java::lang::invoke::MethodHandle>>(accessMode); }

};

} // namespace scapix::java_api

#include <scapix/jni/java_api/macros_pop.h>

#endif // SCAPIX_JAVA_API_JAVA_LANG_INVOKE_INDIRECTVARHANDLE
