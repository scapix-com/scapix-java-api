// Generated by Scapix Java API 2.3
// https://www.scapix.com

#include <scapix/java_api/java/nio/Buffer.h>
#include <scapix/java_api/java/lang/Comparable.h>

#ifndef SCAPIX_JAVA_API_JAVA_NIO_LONGBUFFER_FWD
#define SCAPIX_JAVA_API_JAVA_NIO_LONGBUFFER_FWD

#include <scapix/jni/java_api/macros_push.h>

namespace scapix::java_api {
namespace java::nio {

class LongBuffer;

} // namespace java::nio
} // namespace scapix::java_api

template<>
struct scapix::jni::object_traits<scapix::java_api::java::nio::LongBuffer>
{
	static constexpr fixed_string class_name = "java/nio/LongBuffer";
	using base_classes = std::tuple<scapix::java_api::java::nio::Buffer, scapix::java_api::java::lang::Comparable>;
};

#include <scapix/jni/java_api/macros_pop.h>

#endif // SCAPIX_JAVA_API_JAVA_NIO_LONGBUFFER_FWD

#ifndef SCAPIX_JAVA_API_FWD
#ifndef SCAPIX_JAVA_API_JAVA_NIO_LONGBUFFER
#define SCAPIX_JAVA_API_JAVA_NIO_LONGBUFFER

#define SCAPIX_JAVA_API_FWD
#include <scapix/java_api/java/lang/Object.h>
#include <scapix/java_api/java/lang/String.h>
#include <scapix/java_api/java/nio/ByteOrder.h>
#undef SCAPIX_JAVA_API_FWD

#include <scapix/jni/java_api/macros_push.h>

namespace scapix::java_api {

class java::nio::LongBuffer : public jni::object_base<"java/nio/LongBuffer",
	java::nio::Buffer,
	java::lang::Comparable>
{
public:

	static jni::ref<java::nio::LongBuffer> allocate(jint capacity) { return call_static_method<"allocate", jni::ref<java::nio::LongBuffer>>(capacity); }
	static jni::ref<java::nio::LongBuffer> wrap(jni::ref<jni::array<jlong>> array, jint offset, jint length) { return call_static_method<"wrap", jni::ref<java::nio::LongBuffer>>(array, offset, length); }
	static jni::ref<java::nio::LongBuffer> wrap(jni::ref<jni::array<jlong>> array) { return call_static_method<"wrap", jni::ref<java::nio::LongBuffer>>(array); }
	jni::ref<java::nio::LongBuffer> slice() { return call_method<"slice", jni::ref<java::nio::LongBuffer>>(); }
	jni::ref<java::nio::LongBuffer> slice(jint p1, jint p2) { return call_method<"slice", jni::ref<java::nio::LongBuffer>>(p1, p2); }
	jni::ref<java::nio::LongBuffer> duplicate() { return call_method<"duplicate", jni::ref<java::nio::LongBuffer>>(); }
	jni::ref<java::nio::LongBuffer> asReadOnlyBuffer() { return call_method<"asReadOnlyBuffer", jni::ref<java::nio::LongBuffer>>(); }
	jlong get() { return call_method<"get", jlong>(); }
	jni::ref<java::nio::LongBuffer> put(jlong p1) { return call_method<"put", jni::ref<java::nio::LongBuffer>>(p1); }
	jlong get(jint p1) { return call_method<"get", jlong>(p1); }
	jni::ref<java::nio::LongBuffer> put(jint p1, jlong p2) { return call_method<"put", jni::ref<java::nio::LongBuffer>>(p1, p2); }
	jni::ref<java::nio::LongBuffer> get(jni::ref<jni::array<jlong>> dst, jint offset, jint length) { return call_method<"get", jni::ref<java::nio::LongBuffer>>(dst, offset, length); }
	jni::ref<java::nio::LongBuffer> get(jni::ref<jni::array<jlong>> dst) { return call_method<"get", jni::ref<java::nio::LongBuffer>>(dst); }
	jni::ref<java::nio::LongBuffer> get(jint index, jni::ref<jni::array<jlong>> dst, jint offset, jint length) { return call_method<"get", jni::ref<java::nio::LongBuffer>>(index, dst, offset, length); }
	jni::ref<java::nio::LongBuffer> get(jint index, jni::ref<jni::array<jlong>> dst) { return call_method<"get", jni::ref<java::nio::LongBuffer>>(index, dst); }
	jni::ref<java::nio::LongBuffer> put(jni::ref<java::nio::LongBuffer> src) { return call_method<"put", jni::ref<java::nio::LongBuffer>>(src); }
	jni::ref<java::nio::LongBuffer> put(jint index, jni::ref<java::nio::LongBuffer> src, jint offset, jint length) { return call_method<"put", jni::ref<java::nio::LongBuffer>>(index, src, offset, length); }
	jni::ref<java::nio::LongBuffer> put(jni::ref<jni::array<jlong>> src, jint offset, jint length) { return call_method<"put", jni::ref<java::nio::LongBuffer>>(src, offset, length); }
	jni::ref<java::nio::LongBuffer> put(jni::ref<jni::array<jlong>> src) { return call_method<"put", jni::ref<java::nio::LongBuffer>>(src); }
	jni::ref<java::nio::LongBuffer> put(jint index, jni::ref<jni::array<jlong>> src, jint offset, jint length) { return call_method<"put", jni::ref<java::nio::LongBuffer>>(index, src, offset, length); }
	jni::ref<java::nio::LongBuffer> put(jint index, jni::ref<jni::array<jlong>> src) { return call_method<"put", jni::ref<java::nio::LongBuffer>>(index, src); }
	jboolean hasArray() { return call_method<"hasArray", jboolean>(); }
	jni::ref<jni::array<jlong>> array() { return call_method<"array", jni::ref<jni::array<jlong>>>(); }
	jint arrayOffset() { return call_method<"arrayOffset", jint>(); }
	jni::ref<java::nio::LongBuffer> position(jint newPosition) { return call_method<"position", jni::ref<java::nio::LongBuffer>>(newPosition); }
	jni::ref<java::nio::LongBuffer> limit(jint newLimit) { return call_method<"limit", jni::ref<java::nio::LongBuffer>>(newLimit); }
	jni::ref<java::nio::LongBuffer> mark() { return call_method<"mark", jni::ref<java::nio::LongBuffer>>(); }
	jni::ref<java::nio::LongBuffer> reset() { return call_method<"reset", jni::ref<java::nio::LongBuffer>>(); }
	jni::ref<java::nio::LongBuffer> clear() { return call_method<"clear", jni::ref<java::nio::LongBuffer>>(); }
	jni::ref<java::nio::LongBuffer> flip() { return call_method<"flip", jni::ref<java::nio::LongBuffer>>(); }
	jni::ref<java::nio::LongBuffer> rewind() { return call_method<"rewind", jni::ref<java::nio::LongBuffer>>(); }
	jni::ref<java::nio::LongBuffer> compact() { return call_method<"compact", jni::ref<java::nio::LongBuffer>>(); }
	jboolean isDirect() { return call_method<"isDirect", jboolean>(); }
	jni::ref<java::lang::String> toString() { return call_method<"toString", jni::ref<java::lang::String>>(); }
	jint hashCode() { return call_method<"hashCode", jint>(); }
	jboolean equals(jni::ref<java::lang::Object> ob) { return call_method<"equals", jboolean>(ob); }
	jint compareTo(jni::ref<java::nio::LongBuffer> that) { return call_method<"compareTo", jint>(that); }
	jint mismatch(jni::ref<java::nio::LongBuffer> that) { return call_method<"mismatch", jint>(that); }
	jni::ref<java::nio::ByteOrder> order() { return call_method<"order", jni::ref<java::nio::ByteOrder>>(); }
};

} // namespace scapix::java_api

#include <scapix/jni/java_api/macros_pop.h>

#endif // SCAPIX_JAVA_API_JAVA_NIO_LONGBUFFER
#endif // SCAPIX_JAVA_API_FWD
