// Generated by Scapix Java API 2.3
// https://www.scapix.com

#include <scapix/java_api/java/lang/Object.h>

#ifndef SCAPIX_JAVA_API_ANDROID_COMPANION_ASSOCIATIONREQUEST_BUILDER_FWD
#define SCAPIX_JAVA_API_ANDROID_COMPANION_ASSOCIATIONREQUEST_BUILDER_FWD

#include <scapix/jni/java_api/macros_push.h>

namespace scapix::java_api {
namespace android::companion {

class AssociationRequest_Builder;

} // namespace android::companion
} // namespace scapix::java_api

template<>
struct scapix::jni::object_traits<scapix::java_api::android::companion::AssociationRequest_Builder>
{
	static constexpr fixed_string class_name = "android/companion/AssociationRequest$Builder";
	using base_classes = std::tuple<scapix::java_api::java::lang::Object>;
};

#include <scapix/jni/java_api/macros_pop.h>

#endif // SCAPIX_JAVA_API_ANDROID_COMPANION_ASSOCIATIONREQUEST_BUILDER_FWD

#ifndef SCAPIX_JAVA_API_FWD
#ifndef SCAPIX_JAVA_API_ANDROID_COMPANION_ASSOCIATIONREQUEST_BUILDER
#define SCAPIX_JAVA_API_ANDROID_COMPANION_ASSOCIATIONREQUEST_BUILDER

#define SCAPIX_JAVA_API_FWD
#include <scapix/java_api/android/companion/AssociationRequest.h>
#include <scapix/java_api/android/companion/DeviceFilter.h>
#include <scapix/java_api/android/graphics/drawable/Icon.h>
#include <scapix/java_api/java/lang/CharSequence.h>
#include <scapix/java_api/java/lang/String.h>
#include <scapix/java_api/java/util/Set.h>
#undef SCAPIX_JAVA_API_FWD

#include <scapix/jni/java_api/macros_push.h>

namespace scapix::java_api {

class android::companion::AssociationRequest_Builder : public jni::object_base<"android/companion/AssociationRequest$Builder",
	java::lang::Object>
{
public:

	static jni::ref<android::companion::AssociationRequest_Builder> new_object() { return object_base::new_object(); }
	jni::ref<android::companion::AssociationRequest_Builder> addDeviceFilter(jni::ref<android::companion::DeviceFilter> deviceFilter) { return call_method<"addDeviceFilter", jni::ref<android::companion::AssociationRequest_Builder>>(deviceFilter); }
	jni::ref<android::companion::AssociationRequest> build() { return call_method<"build", jni::ref<android::companion::AssociationRequest>>(); }
	jni::ref<android::companion::AssociationRequest_Builder> setDeviceIcon(jni::ref<android::graphics::drawable::Icon> deviceIcon) { return call_method<"setDeviceIcon", jni::ref<android::companion::AssociationRequest_Builder>>(deviceIcon); }
	jni::ref<android::companion::AssociationRequest_Builder> setDeviceProfile(jni::ref<java::lang::String> deviceProfile) { return call_method<"setDeviceProfile", jni::ref<android::companion::AssociationRequest_Builder>>(deviceProfile); }
	jni::ref<android::companion::AssociationRequest_Builder> setDisplayName(jni::ref<java::lang::CharSequence> displayName) { return call_method<"setDisplayName", jni::ref<android::companion::AssociationRequest_Builder>>(displayName); }
	jni::ref<android::companion::AssociationRequest_Builder> setExtraPermissions(jni::ref<java::util::Set> permissions) { return call_method<"setExtraPermissions", jni::ref<android::companion::AssociationRequest_Builder>>(permissions); }
	jni::ref<android::companion::AssociationRequest_Builder> setForceConfirmation(jboolean forceConfirmation) { return call_method<"setForceConfirmation", jni::ref<android::companion::AssociationRequest_Builder>>(forceConfirmation); }
	jni::ref<android::companion::AssociationRequest_Builder> setRemoteAiAgentSupported(jboolean remoteAiAgentSupported) { return call_method<"setRemoteAiAgentSupported", jni::ref<android::companion::AssociationRequest_Builder>>(remoteAiAgentSupported); }
	jni::ref<android::companion::AssociationRequest_Builder> setSelfManaged(jboolean selfManaged) { return call_method<"setSelfManaged", jni::ref<android::companion::AssociationRequest_Builder>>(selfManaged); }
	jni::ref<android::companion::AssociationRequest_Builder> setSingleDevice(jboolean singleDevice) { return call_method<"setSingleDevice", jni::ref<android::companion::AssociationRequest_Builder>>(singleDevice); }
};

} // namespace scapix::java_api

#include <scapix/jni/java_api/macros_pop.h>

#endif // SCAPIX_JAVA_API_ANDROID_COMPANION_ASSOCIATIONREQUEST_BUILDER
#endif // SCAPIX_JAVA_API_FWD
