// Generated by Scapix Java API 2.3
// https://www.scapix.com

#include <scapix/java_api/java/lang/Object.h>
#include <scapix/java_api/android/os/Parcelable.h>

#ifndef SCAPIX_JAVA_API_ANDROID_TELEPHONY_SUBSCRIPTIONPLAN_FWD
#define SCAPIX_JAVA_API_ANDROID_TELEPHONY_SUBSCRIPTIONPLAN_FWD

#include <scapix/jni/java_api/macros_push.h>

namespace scapix::java_api {
namespace android::telephony {

class SubscriptionPlan;

} // namespace android::telephony
} // namespace scapix::java_api

template<>
struct scapix::jni::object_traits<scapix::java_api::android::telephony::SubscriptionPlan>
{
	static constexpr fixed_string class_name = "android/telephony/SubscriptionPlan";
	using base_classes = std::tuple<scapix::java_api::java::lang::Object, scapix::java_api::android::os::Parcelable>;
};

#include <scapix/jni/java_api/macros_pop.h>

#endif // SCAPIX_JAVA_API_ANDROID_TELEPHONY_SUBSCRIPTIONPLAN_FWD

#ifndef SCAPIX_JAVA_API_FWD
#ifndef SCAPIX_JAVA_API_ANDROID_TELEPHONY_SUBSCRIPTIONPLAN
#define SCAPIX_JAVA_API_ANDROID_TELEPHONY_SUBSCRIPTIONPLAN

#define SCAPIX_JAVA_API_FWD
#include <scapix/java_api/android/os/Parcel.h>
#include <scapix/java_api/android/os/Parcelable_Creator.h>
#include <scapix/java_api/java/lang/CharSequence.h>
#include <scapix/java_api/java/lang/String.h>
#include <scapix/java_api/java/time/ZonedDateTime.h>
#include <scapix/java_api/java/util/Iterator.h>
#include <scapix/java_api/java/util/Set.h>
#include <scapix/java_api/android/telephony/SubscriptionPlan_Builder.h>
#undef SCAPIX_JAVA_API_FWD

#include <scapix/jni/java_api/macros_push.h>

namespace scapix::java_api {

class android::telephony::SubscriptionPlan : public jni::object_base<"android/telephony/SubscriptionPlan",
	java::lang::Object,
	android::os::Parcelable>
{
public:

	using Builder = SubscriptionPlan_Builder;

	static jlong BITRATE_UNKNOWN() { return get_static_field<"BITRATE_UNKNOWN", jlong>(); }
	static jlong BYTES_UNKNOWN() { return get_static_field<"BYTES_UNKNOWN", jlong>(); }
	static jlong BYTES_UNLIMITED() { return get_static_field<"BYTES_UNLIMITED", jlong>(); }
	static jni::ref<android::os::Parcelable_Creator> CREATOR() { return get_static_field<"CREATOR", jni::ref<android::os::Parcelable_Creator>>(); }
	static jint LIMIT_BEHAVIOR_BILLED() { return get_static_field<"LIMIT_BEHAVIOR_BILLED", jint>(); }
	static jint LIMIT_BEHAVIOR_DISABLED() { return get_static_field<"LIMIT_BEHAVIOR_DISABLED", jint>(); }
	static jint LIMIT_BEHAVIOR_THROTTLED() { return get_static_field<"LIMIT_BEHAVIOR_THROTTLED", jint>(); }
	static jint LIMIT_BEHAVIOR_UNKNOWN() { return get_static_field<"LIMIT_BEHAVIOR_UNKNOWN", jint>(); }
	static jint PLAN_TYPE_BUSINESS() { return get_static_field<"PLAN_TYPE_BUSINESS", jint>(); }
	static jint PLAN_TYPE_CELLULAR() { return get_static_field<"PLAN_TYPE_CELLULAR", jint>(); }
	static jint PLAN_TYPE_DATA_ONLY() { return get_static_field<"PLAN_TYPE_DATA_ONLY", jint>(); }
	static jint PLAN_TYPE_FAMILY() { return get_static_field<"PLAN_TYPE_FAMILY", jint>(); }
	static jint PLAN_TYPE_IOT() { return get_static_field<"PLAN_TYPE_IOT", jint>(); }
	static jint PLAN_TYPE_POSTPAID() { return get_static_field<"PLAN_TYPE_POSTPAID", jint>(); }
	static jint PLAN_TYPE_PREPAID() { return get_static_field<"PLAN_TYPE_PREPAID", jint>(); }
	static jint PLAN_TYPE_ROAMING() { return get_static_field<"PLAN_TYPE_ROAMING", jint>(); }
	static jint PLAN_TYPE_SATELLITE() { return get_static_field<"PLAN_TYPE_SATELLITE", jint>(); }
	static jint PLAN_TYPE_TETHERING() { return get_static_field<"PLAN_TYPE_TETHERING", jint>(); }
	static jint SUBSCRIPTION_STATUS_ACTIVE() { return get_static_field<"SUBSCRIPTION_STATUS_ACTIVE", jint>(); }
	static jint SUBSCRIPTION_STATUS_INACTIVE() { return get_static_field<"SUBSCRIPTION_STATUS_INACTIVE", jint>(); }
	static jint SUBSCRIPTION_STATUS_SUSPENDED() { return get_static_field<"SUBSCRIPTION_STATUS_SUSPENDED", jint>(); }
	static jint SUBSCRIPTION_STATUS_TRIAL() { return get_static_field<"SUBSCRIPTION_STATUS_TRIAL", jint>(); }
	static jint SUBSCRIPTION_STATUS_UNKNOWN() { return get_static_field<"SUBSCRIPTION_STATUS_UNKNOWN", jint>(); }
	static jlong TIME_UNKNOWN() { return get_static_field<"TIME_UNKNOWN", jlong>(); }
	static jint UNSPECIFIED_ID() { return get_static_field<"UNSPECIFIED_ID", jint>(); }

	jni::ref<java::util::Iterator> cycleIterator() { return call_method<"cycleIterator", jni::ref<java::util::Iterator>>(); }
	jint describeContents() { return call_method<"describeContents", jint>(); }
	jboolean equals(jni::ref<java::lang::Object> obj) { return call_method<"equals", jboolean>(obj); }
	jint getDataLimitBehavior() { return call_method<"getDataLimitBehavior", jint>(); }
	jlong getDataLimitBytes() { return call_method<"getDataLimitBytes", jlong>(); }
	jlong getDataUsageBytes() { return call_method<"getDataUsageBytes", jlong>(); }
	jni::ref<java::time::ZonedDateTime> getDataUsageResetTime() { return call_method<"getDataUsageResetTime", jni::ref<java::time::ZonedDateTime>>(); }
	jlong getDataUsageTime() { return call_method<"getDataUsageTime", jlong>(); }
	jint getId() { return call_method<"getId", jint>(); }
	jni::ref<jni::array<jint>> getNetworkTypes() { return call_method<"getNetworkTypes", jni::ref<jni::array<jint>>>(); }
	jni::ref<java::time::ZonedDateTime> getPlanEndDate() { return call_method<"getPlanEndDate", jni::ref<java::time::ZonedDateTime>>(); }
	jint getSubscriptionStatus() { return call_method<"getSubscriptionStatus", jint>(); }
	jni::ref<java::lang::CharSequence> getSummary() { return call_method<"getSummary", jni::ref<java::lang::CharSequence>>(); }
	jni::ref<java::lang::CharSequence> getTitle() { return call_method<"getTitle", jni::ref<java::lang::CharSequence>>(); }
	jni::ref<java::util::Set> getTypes() { return call_method<"getTypes", jni::ref<java::util::Set>>(); }
	jint hashCode() { return call_method<"hashCode", jint>(); }
	jni::ref<java::lang::String> toString() { return call_method<"toString", jni::ref<java::lang::String>>(); }
	void writeToParcel(jni::ref<android::os::Parcel> dest, jint flags) { return call_method<"writeToParcel", void>(dest, flags); }
};

} // namespace scapix::java_api

#include <scapix/jni/java_api/macros_pop.h>

#endif // SCAPIX_JAVA_API_ANDROID_TELEPHONY_SUBSCRIPTIONPLAN
#endif // SCAPIX_JAVA_API_FWD
