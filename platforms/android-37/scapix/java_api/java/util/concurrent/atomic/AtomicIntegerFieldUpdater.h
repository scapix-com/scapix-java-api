// Generated by Scapix Java API 2.3
// https://www.scapix.com

#include <scapix/java_api/java/lang/Object.h>

#ifndef SCAPIX_JAVA_API_JAVA_UTIL_CONCURRENT_ATOMIC_ATOMICINTEGERFIELDUPDATER_FWD
#define SCAPIX_JAVA_API_JAVA_UTIL_CONCURRENT_ATOMIC_ATOMICINTEGERFIELDUPDATER_FWD

#include <scapix/jni/java_api/macros_push.h>

namespace scapix::java_api {
namespace java::util::concurrent::atomic {

class AtomicIntegerFieldUpdater;

} // namespace java::util::concurrent::atomic
} // namespace scapix::java_api

template<>
struct scapix::jni::object_traits<scapix::java_api::java::util::concurrent::atomic::AtomicIntegerFieldUpdater>
{
	static constexpr fixed_string class_name = "java/util/concurrent/atomic/AtomicIntegerFieldUpdater";
	using base_classes = std::tuple<scapix::java_api::java::lang::Object>;
};

#include <scapix/jni/java_api/macros_pop.h>

#endif // SCAPIX_JAVA_API_JAVA_UTIL_CONCURRENT_ATOMIC_ATOMICINTEGERFIELDUPDATER_FWD

#ifndef SCAPIX_JAVA_API_FWD
#ifndef SCAPIX_JAVA_API_JAVA_UTIL_CONCURRENT_ATOMIC_ATOMICINTEGERFIELDUPDATER
#define SCAPIX_JAVA_API_JAVA_UTIL_CONCURRENT_ATOMIC_ATOMICINTEGERFIELDUPDATER

#define SCAPIX_JAVA_API_FWD
#include <scapix/java_api/java/lang/Class.h>
#include <scapix/java_api/java/lang/String.h>
#include <scapix/java_api/java/util/function/IntBinaryOperator.h>
#include <scapix/java_api/java/util/function/IntUnaryOperator.h>
#undef SCAPIX_JAVA_API_FWD

#include <scapix/jni/java_api/macros_push.h>

namespace scapix::java_api {

class java::util::concurrent::atomic::AtomicIntegerFieldUpdater : public jni::object_base<"java/util/concurrent/atomic/AtomicIntegerFieldUpdater",
	java::lang::Object>
{
public:

	jint accumulateAndGet(jni::ref<java::lang::Object> obj, jint x, jni::ref<java::util::function::IntBinaryOperator> accumulatorFunction) { return call_method<"accumulateAndGet", jint>(obj, x, accumulatorFunction); }
	jint addAndGet(jni::ref<java::lang::Object> obj, jint delta) { return call_method<"addAndGet", jint>(obj, delta); }
	jboolean compareAndSet(jni::ref<java::lang::Object> p1, jint p2, jint p3) { return call_method<"compareAndSet", jboolean>(p1, p2, p3); }
	jint decrementAndGet(jni::ref<java::lang::Object> obj) { return call_method<"decrementAndGet", jint>(obj); }
	jint get(jni::ref<java::lang::Object> p1) { return call_method<"get", jint>(p1); }
	jint getAndAccumulate(jni::ref<java::lang::Object> obj, jint x, jni::ref<java::util::function::IntBinaryOperator> accumulatorFunction) { return call_method<"getAndAccumulate", jint>(obj, x, accumulatorFunction); }
	jint getAndAdd(jni::ref<java::lang::Object> obj, jint delta) { return call_method<"getAndAdd", jint>(obj, delta); }
	jint getAndDecrement(jni::ref<java::lang::Object> obj) { return call_method<"getAndDecrement", jint>(obj); }
	jint getAndIncrement(jni::ref<java::lang::Object> obj) { return call_method<"getAndIncrement", jint>(obj); }
	jint getAndSet(jni::ref<java::lang::Object> obj, jint newValue) { return call_method<"getAndSet", jint>(obj, newValue); }
	jint getAndUpdate(jni::ref<java::lang::Object> obj, jni::ref<java::util::function::IntUnaryOperator> updateFunction) { return call_method<"getAndUpdate", jint>(obj, updateFunction); }
	jint incrementAndGet(jni::ref<java::lang::Object> obj) { return call_method<"incrementAndGet", jint>(obj); }
	void lazySet(jni::ref<java::lang::Object> p1, jint p2) { return call_method<"lazySet", void>(p1, p2); }
	static jni::ref<java::util::concurrent::atomic::AtomicIntegerFieldUpdater> newUpdater(jni::ref<java::lang::Class> tclass, jni::ref<java::lang::String> fieldName) { return call_static_method<"newUpdater", jni::ref<java::util::concurrent::atomic::AtomicIntegerFieldUpdater>>(tclass, fieldName); }
	void set(jni::ref<java::lang::Object> p1, jint p2) { return call_method<"set", void>(p1, p2); }
	jint updateAndGet(jni::ref<java::lang::Object> obj, jni::ref<java::util::function::IntUnaryOperator> updateFunction) { return call_method<"updateAndGet", jint>(obj, updateFunction); }
	jboolean weakCompareAndSet(jni::ref<java::lang::Object> p1, jint p2, jint p3) { return call_method<"weakCompareAndSet", jboolean>(p1, p2, p3); }
};

} // namespace scapix::java_api

#include <scapix/jni/java_api/macros_pop.h>

#endif // SCAPIX_JAVA_API_JAVA_UTIL_CONCURRENT_ATOMIC_ATOMICINTEGERFIELDUPDATER
#endif // SCAPIX_JAVA_API_FWD
