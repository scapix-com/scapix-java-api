// Generated by Scapix Java API 2.3
// https://www.scapix.com

#include <scapix/java_api/java/lang/Enum.h>

#ifndef SCAPIX_JAVA_API_JAVA_UTIL_CONCURRENT_TIMEUNIT_FWD
#define SCAPIX_JAVA_API_JAVA_UTIL_CONCURRENT_TIMEUNIT_FWD

#include <scapix/jni/java_api/macros_push.h>

namespace scapix::java_api {
namespace java::util::concurrent {

class TimeUnit;

} // namespace java::util::concurrent
} // namespace scapix::java_api

template<>
struct scapix::jni::object_traits<scapix::java_api::java::util::concurrent::TimeUnit>
{
	static constexpr fixed_string class_name = "java/util/concurrent/TimeUnit";
	using base_classes = std::tuple<scapix::java_api::java::lang::Enum>;
};

#include <scapix/jni/java_api/macros_pop.h>

#endif // SCAPIX_JAVA_API_JAVA_UTIL_CONCURRENT_TIMEUNIT_FWD

#ifndef SCAPIX_JAVA_API_FWD
#ifndef SCAPIX_JAVA_API_JAVA_UTIL_CONCURRENT_TIMEUNIT
#define SCAPIX_JAVA_API_JAVA_UTIL_CONCURRENT_TIMEUNIT

#define SCAPIX_JAVA_API_FWD
#include <scapix/java_api/java/lang/Object.h>
#include <scapix/java_api/java/lang/String.h>
#include <scapix/java_api/java/lang/Thread.h>
#include <scapix/java_api/java/time/Duration.h>
#include <scapix/java_api/java/time/temporal/ChronoUnit.h>
#undef SCAPIX_JAVA_API_FWD

#include <scapix/jni/java_api/macros_push.h>

namespace scapix::java_api {

class java::util::concurrent::TimeUnit : public jni::object_base<"java/util/concurrent/TimeUnit",
	java::lang::Enum>
{
public:

	static jni::ref<java::util::concurrent::TimeUnit> DAYS() { return get_static_field<"DAYS", jni::ref<java::util::concurrent::TimeUnit>>(); }
	static jni::ref<java::util::concurrent::TimeUnit> HOURS() { return get_static_field<"HOURS", jni::ref<java::util::concurrent::TimeUnit>>(); }
	static jni::ref<java::util::concurrent::TimeUnit> MICROSECONDS() { return get_static_field<"MICROSECONDS", jni::ref<java::util::concurrent::TimeUnit>>(); }
	static jni::ref<java::util::concurrent::TimeUnit> MILLISECONDS() { return get_static_field<"MILLISECONDS", jni::ref<java::util::concurrent::TimeUnit>>(); }
	static jni::ref<java::util::concurrent::TimeUnit> MINUTES() { return get_static_field<"MINUTES", jni::ref<java::util::concurrent::TimeUnit>>(); }
	static jni::ref<java::util::concurrent::TimeUnit> NANOSECONDS() { return get_static_field<"NANOSECONDS", jni::ref<java::util::concurrent::TimeUnit>>(); }
	static jni::ref<java::util::concurrent::TimeUnit> SECONDS() { return get_static_field<"SECONDS", jni::ref<java::util::concurrent::TimeUnit>>(); }

	static jni::ref<jni::array<java::util::concurrent::TimeUnit>> values() { return call_static_method<"values", jni::ref<jni::array<java::util::concurrent::TimeUnit>>>(); }
	static jni::ref<java::util::concurrent::TimeUnit> valueOf(jni::ref<java::lang::String> name) { return call_static_method<"valueOf", jni::ref<java::util::concurrent::TimeUnit>>(name); }
	jlong convert(jni::ref<java::time::Duration> duration) { return call_method<"convert", jlong>(duration); }
	jlong convert(jlong sourceDuration, jni::ref<java::util::concurrent::TimeUnit> sourceUnit) { return call_method<"convert", jlong>(sourceDuration, sourceUnit); }
	static jni::ref<java::util::concurrent::TimeUnit> of(jni::ref<java::time::temporal::ChronoUnit> chronoUnit) { return call_static_method<"of", jni::ref<java::util::concurrent::TimeUnit>>(chronoUnit); }
	void sleep(jlong timeout) { return call_method<"sleep", void>(timeout); }
	void timedJoin(jni::ref<java::lang::Thread> thread, jlong timeout) { return call_method<"timedJoin", void>(thread, timeout); }
	void timedWait(jni::ref<java::lang::Object> obj, jlong timeout) { return call_method<"timedWait", void>(obj, timeout); }
	jni::ref<java::time::temporal::ChronoUnit> toChronoUnit() { return call_method<"toChronoUnit", jni::ref<java::time::temporal::ChronoUnit>>(); }
	jlong toDays(jlong duration) { return call_method<"toDays", jlong>(duration); }
	jlong toHours(jlong duration) { return call_method<"toHours", jlong>(duration); }
	jlong toMicros(jlong duration) { return call_method<"toMicros", jlong>(duration); }
	jlong toMillis(jlong duration) { return call_method<"toMillis", jlong>(duration); }
	jlong toMinutes(jlong duration) { return call_method<"toMinutes", jlong>(duration); }
	jlong toNanos(jlong duration) { return call_method<"toNanos", jlong>(duration); }
	jlong toSeconds(jlong duration) { return call_method<"toSeconds", jlong>(duration); }
};

} // namespace scapix::java_api

#include <scapix/jni/java_api/macros_pop.h>

#endif // SCAPIX_JAVA_API_JAVA_UTIL_CONCURRENT_TIMEUNIT
#endif // SCAPIX_JAVA_API_FWD
