// Generated by Scapix Java API 2.3
// https://www.scapix.com

#include <scapix/java_api/java/lang/Object.h>

#ifndef SCAPIX_JAVA_API_ANDROID_TEXT_LAYOUT_FWD
#define SCAPIX_JAVA_API_ANDROID_TEXT_LAYOUT_FWD

#include <scapix/jni/java_api/macros_push.h>

namespace scapix::java_api {
namespace android::text {

class Layout;

} // namespace android::text
} // namespace scapix::java_api

template<>
struct scapix::jni::object_traits<scapix::java_api::android::text::Layout>
{
	static constexpr fixed_string class_name = "android/text/Layout";
	using base_classes = std::tuple<scapix::java_api::java::lang::Object>;
};

#include <scapix/jni/java_api/macros_pop.h>

#endif // SCAPIX_JAVA_API_ANDROID_TEXT_LAYOUT_FWD

#ifndef SCAPIX_JAVA_API_FWD
#ifndef SCAPIX_JAVA_API_ANDROID_TEXT_LAYOUT
#define SCAPIX_JAVA_API_ANDROID_TEXT_LAYOUT

#define SCAPIX_JAVA_API_FWD
#include <scapix/java_api/android/graphics/Canvas.h>
#include <scapix/java_api/android/graphics/Paint.h>
#include <scapix/java_api/android/graphics/Paint_FontMetrics.h>
#include <scapix/java_api/android/graphics/Path.h>
#include <scapix/java_api/android/graphics/Rect.h>
#include <scapix/java_api/android/graphics/RectF.h>
#include <scapix/java_api/android/graphics/text/LineBreakConfig.h>
#include <scapix/java_api/android/text/Layout_Alignment.h>
#include <scapix/java_api/android/text/Layout_Directions.h>
#include <scapix/java_api/android/text/Layout_TextInclusionStrategy.h>
#include <scapix/java_api/android/text/SegmentFinder.h>
#include <scapix/java_api/android/text/TextDirectionHeuristic.h>
#include <scapix/java_api/android/text/TextPaint.h>
#include <scapix/java_api/android/text/TextUtils_TruncateAt.h>
#include <scapix/java_api/java/lang/CharSequence.h>
#include <scapix/java_api/java/util/List.h>
#include <scapix/java_api/android/text/Layout_Builder.h>
#undef SCAPIX_JAVA_API_FWD

#include <scapix/jni/java_api/macros_push.h>

namespace scapix::java_api {

class android::text::Layout : public jni::object_base<"android/text/Layout",
	java::lang::Object>
{
public:

	using TextInclusionStrategy = Layout_TextInclusionStrategy;
	using Directions = Layout_Directions;
	using Builder = Layout_Builder;
	using Alignment = Layout_Alignment;

	static jint BREAK_STRATEGY_BALANCED() { return get_static_field<"BREAK_STRATEGY_BALANCED", jint>(); }
	static jint BREAK_STRATEGY_HIGH_QUALITY() { return get_static_field<"BREAK_STRATEGY_HIGH_QUALITY", jint>(); }
	static jint BREAK_STRATEGY_SIMPLE() { return get_static_field<"BREAK_STRATEGY_SIMPLE", jint>(); }
	static jfloat DEFAULT_LINESPACING_ADDITION() { return get_static_field<"DEFAULT_LINESPACING_ADDITION", jfloat>(); }
	static jfloat DEFAULT_LINESPACING_MULTIPLIER() { return get_static_field<"DEFAULT_LINESPACING_MULTIPLIER", jfloat>(); }
	static jint DIR_LEFT_TO_RIGHT() { return get_static_field<"DIR_LEFT_TO_RIGHT", jint>(); }
	static jint DIR_RIGHT_TO_LEFT() { return get_static_field<"DIR_RIGHT_TO_LEFT", jint>(); }
	static jint HYPHENATION_FREQUENCY_FULL() { return get_static_field<"HYPHENATION_FREQUENCY_FULL", jint>(); }
	static jint HYPHENATION_FREQUENCY_FULL_FAST() { return get_static_field<"HYPHENATION_FREQUENCY_FULL_FAST", jint>(); }
	static jint HYPHENATION_FREQUENCY_NONE() { return get_static_field<"HYPHENATION_FREQUENCY_NONE", jint>(); }
	static jint HYPHENATION_FREQUENCY_NORMAL() { return get_static_field<"HYPHENATION_FREQUENCY_NORMAL", jint>(); }
	static jint HYPHENATION_FREQUENCY_NORMAL_FAST() { return get_static_field<"HYPHENATION_FREQUENCY_NORMAL_FAST", jint>(); }
	static jni::ref<android::text::Layout_TextInclusionStrategy> INCLUSION_STRATEGY_ANY_OVERLAP() { return get_static_field<"INCLUSION_STRATEGY_ANY_OVERLAP", jni::ref<android::text::Layout_TextInclusionStrategy>>(); }
	static jni::ref<android::text::Layout_TextInclusionStrategy> INCLUSION_STRATEGY_CONTAINS_ALL() { return get_static_field<"INCLUSION_STRATEGY_CONTAINS_ALL", jni::ref<android::text::Layout_TextInclusionStrategy>>(); }
	static jni::ref<android::text::Layout_TextInclusionStrategy> INCLUSION_STRATEGY_CONTAINS_CENTER() { return get_static_field<"INCLUSION_STRATEGY_CONTAINS_CENTER", jni::ref<android::text::Layout_TextInclusionStrategy>>(); }
	static jint JUSTIFICATION_MODE_INTER_CHARACTER() { return get_static_field<"JUSTIFICATION_MODE_INTER_CHARACTER", jint>(); }
	static jint JUSTIFICATION_MODE_INTER_WORD() { return get_static_field<"JUSTIFICATION_MODE_INTER_WORD", jint>(); }
	static jint JUSTIFICATION_MODE_NONE() { return get_static_field<"JUSTIFICATION_MODE_NONE", jint>(); }

	static jfloat getDesiredWidth(jni::ref<java::lang::CharSequence> source, jni::ref<android::text::TextPaint> paint) { return call_static_method<"getDesiredWidth", jfloat>(source, paint); }
	static jfloat getDesiredWidth(jni::ref<java::lang::CharSequence> source, jint start, jint end, jni::ref<android::text::TextPaint> paint) { return call_static_method<"getDesiredWidth", jfloat>(source, start, end, paint); }
	void draw(jni::ref<android::graphics::Canvas> c) { return call_method<"draw", void>(c); }
	void draw(jni::ref<android::graphics::Canvas> canvas, jni::ref<android::graphics::Path> selectionHighlight, jni::ref<android::graphics::Paint> selectionHighlightPaint, jint cursorOffsetVertical) { return call_method<"draw", void>(canvas, selectionHighlight, selectionHighlightPaint, cursorOffsetVertical); }
	void draw(jni::ref<android::graphics::Canvas> canvas, jni::ref<java::util::List> highlightPaths, jni::ref<java::util::List> highlightPaints, jni::ref<android::graphics::Path> selectionPath, jni::ref<android::graphics::Paint> selectionPaint, jint cursorOffsetVertical) { return call_method<"draw", void>(canvas, highlightPaths, highlightPaints, selectionPath, selectionPaint, cursorOffsetVertical); }
	void drawText(jni::ref<android::graphics::Canvas> canvas) { return call_method<"drawText", void>(canvas); }
	void drawBackground(jni::ref<android::graphics::Canvas> canvas) { return call_method<"drawBackground", void>(canvas); }
	void increaseWidthTo(jint wid) { return call_method<"increaseWidthTo", void>(wid); }
	jint getHeight() { return call_method<"getHeight", jint>(); }
	jint getLineCount() { return call_method<"getLineCount", jint>(); }
	jni::ref<android::graphics::RectF> computeDrawingBoundingBox() { return call_method<"computeDrawingBoundingBox", jni::ref<android::graphics::RectF>>(); }
	jint getLineBounds(jint line, jni::ref<android::graphics::Rect> bounds) { return call_method<"getLineBounds", jint>(line, bounds); }
	jint getLineTop(jint p1) { return call_method<"getLineTop", jint>(p1); }
	jint getLineDescent(jint p1) { return call_method<"getLineDescent", jint>(p1); }
	jint getLineStart(jint p1) { return call_method<"getLineStart", jint>(p1); }
	jint getParagraphDirection(jint p1) { return call_method<"getParagraphDirection", jint>(p1); }
	jboolean getLineContainsTab(jint p1) { return call_method<"getLineContainsTab", jboolean>(p1); }
	jni::ref<android::text::Layout_Directions> getLineDirections(jint p1) { return call_method<"getLineDirections", jni::ref<android::text::Layout_Directions>>(p1); }
	jint getTopPadding() { return call_method<"getTopPadding", jint>(); }
	jint getBottomPadding() { return call_method<"getBottomPadding", jint>(); }
	jboolean isRtlCharAt(jint offset) { return call_method<"isRtlCharAt", jboolean>(offset); }
	jfloat getPrimaryHorizontal(jint offset) { return call_method<"getPrimaryHorizontal", jfloat>(offset); }
	jfloat getSecondaryHorizontal(jint offset) { return call_method<"getSecondaryHorizontal", jfloat>(offset); }
	void fillCharacterBounds(jint start, jint end, jni::ref<jni::array<jfloat>> bounds, jint boundsStart) { return call_method<"fillCharacterBounds", void>(start, end, bounds, boundsStart); }
	jfloat getLineLeft(jint line) { return call_method<"getLineLeft", jfloat>(line); }
	jfloat getLineRight(jint line) { return call_method<"getLineRight", jfloat>(line); }
	jfloat getLineMax(jint line) { return call_method<"getLineMax", jfloat>(line); }
	jfloat getLineWidth(jint line) { return call_method<"getLineWidth", jfloat>(line); }
	jint getLineLetterSpacingUnitCount(jint line, jboolean includeTrailingWhitespace) { return call_method<"getLineLetterSpacingUnitCount", jint>(line, includeTrailingWhitespace); }
	jint getLineForVertical(jint vertical) { return call_method<"getLineForVertical", jint>(vertical); }
	jint getLineForOffset(jint offset) { return call_method<"getLineForOffset", jint>(offset); }
	jint getOffsetForHorizontal(jint line, jfloat horiz) { return call_method<"getOffsetForHorizontal", jint>(line, horiz); }
	jni::ref<jni::array<jint>> getRangeForRect(jni::ref<android::graphics::RectF> area, jni::ref<android::text::SegmentFinder> segmentFinder, jni::ref<android::text::Layout_TextInclusionStrategy> inclusionStrategy) { return call_method<"getRangeForRect", jni::ref<jni::array<jint>>>(area, segmentFinder, inclusionStrategy); }
	jint getLineEnd(jint line) { return call_method<"getLineEnd", jint>(line); }
	jint getLineVisibleEnd(jint line) { return call_method<"getLineVisibleEnd", jint>(line); }
	jint getLineBottom(jint line) { return call_method<"getLineBottom", jint>(line); }
	jint getLineBottom(jint line, jboolean includeLineSpacing) { return call_method<"getLineBottom", jint>(line, includeLineSpacing); }
	jint getLineBaseline(jint line) { return call_method<"getLineBaseline", jint>(line); }
	jint getLineAscent(jint line) { return call_method<"getLineAscent", jint>(line); }
	jint getOffsetToLeftOf(jint offset) { return call_method<"getOffsetToLeftOf", jint>(offset); }
	jint getOffsetToRightOf(jint offset) { return call_method<"getOffsetToRightOf", jint>(offset); }
	void getCursorPath(jint point, jni::ref<android::graphics::Path> dest, jni::ref<java::lang::CharSequence> editingBuffer) { return call_method<"getCursorPath", void>(point, dest, editingBuffer); }
	void getSelectionPath(jint start, jint end, jni::ref<android::graphics::Path> dest) { return call_method<"getSelectionPath", void>(start, end, dest); }
	jni::ref<android::text::Layout_Alignment> getParagraphAlignment(jint line) { return call_method<"getParagraphAlignment", jni::ref<android::text::Layout_Alignment>>(line); }
	jint getParagraphLeft(jint line) { return call_method<"getParagraphLeft", jint>(line); }
	jint getParagraphRight(jint line) { return call_method<"getParagraphRight", jint>(line); }
	jint getEllipsisStart(jint p1) { return call_method<"getEllipsisStart", jint>(p1); }
	jint getEllipsisCount(jint p1) { return call_method<"getEllipsisCount", jint>(p1); }
	jni::ref<java::lang::CharSequence> getText() { return call_method<"getText", jni::ref<java::lang::CharSequence>>(); }
	jni::ref<android::text::TextPaint> getPaint() { return call_method<"getPaint", jni::ref<android::text::TextPaint>>(); }
	jint getWidth() { return call_method<"getWidth", jint>(); }
	jni::ref<android::text::Layout_Alignment> getAlignment() { return call_method<"getAlignment", jni::ref<android::text::Layout_Alignment>>(); }
	jni::ref<android::text::TextDirectionHeuristic> getTextDirectionHeuristic() { return call_method<"getTextDirectionHeuristic", jni::ref<android::text::TextDirectionHeuristic>>(); }
	jfloat getSpacingMultiplier() { return call_method<"getSpacingMultiplier", jfloat>(); }
	jfloat getLineSpacingMultiplier() { return call_method<"getLineSpacingMultiplier", jfloat>(); }
	jfloat getSpacingAdd() { return call_method<"getSpacingAdd", jfloat>(); }
	jfloat getLineSpacingAmount() { return call_method<"getLineSpacingAmount", jfloat>(); }
	jboolean isFontPaddingIncluded() { return call_method<"isFontPaddingIncluded", jboolean>(); }
	jboolean isFallbackLineSpacingEnabled() { return call_method<"isFallbackLineSpacingEnabled", jboolean>(); }
	jint getEllipsizedWidth() { return call_method<"getEllipsizedWidth", jint>(); }
	jni::ref<android::text::TextUtils_TruncateAt> getEllipsize() { return call_method<"getEllipsize", jni::ref<android::text::TextUtils_TruncateAt>>(); }
	jint getMaxLines() { return call_method<"getMaxLines", jint>(); }
	jint getBreakStrategy() { return call_method<"getBreakStrategy", jint>(); }
	jint getHyphenationFrequency() { return call_method<"getHyphenationFrequency", jint>(); }
	jni::ref<jni::array<jint>> getLeftIndents() { return call_method<"getLeftIndents", jni::ref<jni::array<jint>>>(); }
	jni::ref<jni::array<jint>> getRightIndents() { return call_method<"getRightIndents", jni::ref<jni::array<jint>>>(); }
	jint getJustificationMode() { return call_method<"getJustificationMode", jint>(); }
	jni::ref<android::graphics::text::LineBreakConfig> getLineBreakConfig() { return call_method<"getLineBreakConfig", jni::ref<android::graphics::text::LineBreakConfig>>(); }
	jboolean getUseBoundsForWidth() { return call_method<"getUseBoundsForWidth", jboolean>(); }
	jboolean getShiftDrawingOffsetForStartOverhang() { return call_method<"getShiftDrawingOffsetForStartOverhang", jboolean>(); }
	jni::ref<android::graphics::Paint_FontMetrics> getMinimumFontMetrics() { return call_method<"getMinimumFontMetrics", jni::ref<android::graphics::Paint_FontMetrics>>(); }
};

} // namespace scapix::java_api

#include <scapix/jni/java_api/macros_pop.h>

#endif // SCAPIX_JAVA_API_ANDROID_TEXT_LAYOUT
#endif // SCAPIX_JAVA_API_FWD
