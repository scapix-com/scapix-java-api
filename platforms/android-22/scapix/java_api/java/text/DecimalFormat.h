// Generated by Scapix Java API 2.3
// https://www.scapix.com

#include <scapix/java_api/java/text/NumberFormat.h>

#ifndef SCAPIX_JAVA_API_JAVA_TEXT_DECIMALFORMAT_FWD
#define SCAPIX_JAVA_API_JAVA_TEXT_DECIMALFORMAT_FWD

#include <scapix/jni/java_api/macros_push.h>

namespace scapix::java_api {
namespace java::text {

class DecimalFormat;

} // namespace java::text
} // namespace scapix::java_api

template<>
struct scapix::jni::object_traits<scapix::java_api::java::text::DecimalFormat>
{
	static constexpr fixed_string class_name = "java/text/DecimalFormat";
	using base_classes = std::tuple<scapix::java_api::java::text::NumberFormat>;
};

#include <scapix/jni/java_api/macros_pop.h>

#endif // SCAPIX_JAVA_API_JAVA_TEXT_DECIMALFORMAT_FWD

#ifndef SCAPIX_JAVA_API_FWD
#ifndef SCAPIX_JAVA_API_JAVA_TEXT_DECIMALFORMAT
#define SCAPIX_JAVA_API_JAVA_TEXT_DECIMALFORMAT

#define SCAPIX_JAVA_API_FWD
#include <scapix/java_api/java/lang/Number.h>
#include <scapix/java_api/java/lang/Object.h>
#include <scapix/java_api/java/lang/String.h>
#include <scapix/java_api/java/lang/StringBuffer.h>
#include <scapix/java_api/java/math/RoundingMode.h>
#include <scapix/java_api/java/text/AttributedCharacterIterator.h>
#include <scapix/java_api/java/text/DecimalFormatSymbols.h>
#include <scapix/java_api/java/text/FieldPosition.h>
#include <scapix/java_api/java/text/ParsePosition.h>
#include <scapix/java_api/java/util/Currency.h>
#undef SCAPIX_JAVA_API_FWD

#include <scapix/jni/java_api/macros_push.h>

namespace scapix::java_api {

class java::text::DecimalFormat : public jni::object_base<"java/text/DecimalFormat",
	java::text::NumberFormat>
{
public:

	static jni::ref<java::text::DecimalFormat> new_object() { return object_base::new_object(); }
	static jni::ref<java::text::DecimalFormat> new_object(jni::ref<java::lang::String> pattern) { return object_base::new_object(pattern); }
	static jni::ref<java::text::DecimalFormat> new_object(jni::ref<java::lang::String> pattern, jni::ref<java::text::DecimalFormatSymbols> value) { return object_base::new_object(pattern, value); }
	void applyLocalizedPattern(jni::ref<java::lang::String> pattern) { return call_method<"applyLocalizedPattern", void>(pattern); }
	void applyPattern(jni::ref<java::lang::String> pattern) { return call_method<"applyPattern", void>(pattern); }
	jni::ref<java::lang::Object> clone() { return call_method<"clone", jni::ref<java::lang::Object>>(); }
	jboolean equals(jni::ref<java::lang::Object> object) { return call_method<"equals", jboolean>(object); }
	jni::ref<java::text::AttributedCharacterIterator> formatToCharacterIterator(jni::ref<java::lang::Object> object) { return call_method<"formatToCharacterIterator", jni::ref<java::text::AttributedCharacterIterator>>(object); }
	jni::ref<java::lang::StringBuffer> format(jdouble value, jni::ref<java::lang::StringBuffer> buffer, jni::ref<java::text::FieldPosition> position) { return call_method<"format", jni::ref<java::lang::StringBuffer>>(value, buffer, position); }
	jni::ref<java::lang::StringBuffer> format(jlong value, jni::ref<java::lang::StringBuffer> buffer, jni::ref<java::text::FieldPosition> position) { return call_method<"format", jni::ref<java::lang::StringBuffer>>(value, buffer, position); }
	jni::ref<java::lang::StringBuffer> format(jni::ref<java::lang::Object> number, jni::ref<java::lang::StringBuffer> buffer, jni::ref<java::text::FieldPosition> position) { return call_method<"format", jni::ref<java::lang::StringBuffer>>(number, buffer, position); }
	jni::ref<java::text::DecimalFormatSymbols> getDecimalFormatSymbols() { return call_method<"getDecimalFormatSymbols", jni::ref<java::text::DecimalFormatSymbols>>(); }
	jni::ref<java::util::Currency> getCurrency() { return call_method<"getCurrency", jni::ref<java::util::Currency>>(); }
	jint getGroupingSize() { return call_method<"getGroupingSize", jint>(); }
	jni::ref<java::lang::String> getNegativePrefix() { return call_method<"getNegativePrefix", jni::ref<java::lang::String>>(); }
	jni::ref<java::lang::String> getNegativeSuffix() { return call_method<"getNegativeSuffix", jni::ref<java::lang::String>>(); }
	jni::ref<java::lang::String> getPositivePrefix() { return call_method<"getPositivePrefix", jni::ref<java::lang::String>>(); }
	jni::ref<java::lang::String> getPositiveSuffix() { return call_method<"getPositiveSuffix", jni::ref<java::lang::String>>(); }
	jint hashCode() { return call_method<"hashCode", jint>(); }
	jboolean isDecimalSeparatorAlwaysShown() { return call_method<"isDecimalSeparatorAlwaysShown", jboolean>(); }
	jboolean isParseBigDecimal() { return call_method<"isParseBigDecimal", jboolean>(); }
	void setParseIntegerOnly(jboolean value) { return call_method<"setParseIntegerOnly", void>(value); }
	jboolean isParseIntegerOnly() { return call_method<"isParseIntegerOnly", jboolean>(); }
	jni::ref<java::lang::Number> parse(jni::ref<java::lang::String> string, jni::ref<java::text::ParsePosition> position) { return call_method<"parse", jni::ref<java::lang::Number>>(string, position); }
	void setDecimalFormatSymbols(jni::ref<java::text::DecimalFormatSymbols> value) { return call_method<"setDecimalFormatSymbols", void>(value); }
	void setCurrency(jni::ref<java::util::Currency> currency) { return call_method<"setCurrency", void>(currency); }
	void setDecimalSeparatorAlwaysShown(jboolean value) { return call_method<"setDecimalSeparatorAlwaysShown", void>(value); }
	void setGroupingSize(jint value) { return call_method<"setGroupingSize", void>(value); }
	void setGroupingUsed(jboolean value) { return call_method<"setGroupingUsed", void>(value); }
	jboolean isGroupingUsed() { return call_method<"isGroupingUsed", jboolean>(); }
	void setMaximumFractionDigits(jint value) { return call_method<"setMaximumFractionDigits", void>(value); }
	void setMaximumIntegerDigits(jint value) { return call_method<"setMaximumIntegerDigits", void>(value); }
	void setMinimumFractionDigits(jint value) { return call_method<"setMinimumFractionDigits", void>(value); }
	void setMinimumIntegerDigits(jint value) { return call_method<"setMinimumIntegerDigits", void>(value); }
	jint getMultiplier() { return call_method<"getMultiplier", jint>(); }
	void setMultiplier(jint value) { return call_method<"setMultiplier", void>(value); }
	void setNegativePrefix(jni::ref<java::lang::String> value) { return call_method<"setNegativePrefix", void>(value); }
	void setNegativeSuffix(jni::ref<java::lang::String> value) { return call_method<"setNegativeSuffix", void>(value); }
	void setPositivePrefix(jni::ref<java::lang::String> value) { return call_method<"setPositivePrefix", void>(value); }
	void setPositiveSuffix(jni::ref<java::lang::String> value) { return call_method<"setPositiveSuffix", void>(value); }
	void setParseBigDecimal(jboolean newValue) { return call_method<"setParseBigDecimal", void>(newValue); }
	jni::ref<java::lang::String> toLocalizedPattern() { return call_method<"toLocalizedPattern", jni::ref<java::lang::String>>(); }
	jni::ref<java::lang::String> toPattern() { return call_method<"toPattern", jni::ref<java::lang::String>>(); }
	jni::ref<java::math::RoundingMode> getRoundingMode() { return call_method<"getRoundingMode", jni::ref<java::math::RoundingMode>>(); }
	void setRoundingMode(jni::ref<java::math::RoundingMode> roundingMode) { return call_method<"setRoundingMode", void>(roundingMode); }
	jni::ref<java::lang::String> toString() { return call_method<"toString", jni::ref<java::lang::String>>(); }
};

} // namespace scapix::java_api

#include <scapix/jni/java_api/macros_pop.h>

#endif // SCAPIX_JAVA_API_JAVA_TEXT_DECIMALFORMAT
#endif // SCAPIX_JAVA_API_FWD
