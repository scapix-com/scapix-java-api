// Generated by Scapix Java API 2.3
// https://www.scapix.com

#include <scapix/java_api/java/security/BasicPermission.h>
#include <scapix/java_api/java/security/Guard.h>
#include <scapix/java_api/java/io/Serializable.h>

#ifndef SCAPIX_JAVA_API_JAVA_UTIL_LOGGING_LOGGINGPERMISSION_FWD
#define SCAPIX_JAVA_API_JAVA_UTIL_LOGGING_LOGGINGPERMISSION_FWD

#include <scapix/jni/java_api/macros_push.h>

namespace scapix::java_api {
namespace java::util::logging {

class LoggingPermission;

} // namespace java::util::logging
} // namespace scapix::java_api

template<>
struct scapix::jni::object_traits<scapix::java_api::java::util::logging::LoggingPermission>
{
	static constexpr fixed_string class_name = "java/util/logging/LoggingPermission";
	using base_classes = std::tuple<scapix::java_api::java::security::BasicPermission, scapix::java_api::java::security::Guard, scapix::java_api::java::io::Serializable>;
};

#include <scapix/jni/java_api/macros_pop.h>

#endif // SCAPIX_JAVA_API_JAVA_UTIL_LOGGING_LOGGINGPERMISSION_FWD

#ifndef SCAPIX_JAVA_API_FWD
#ifndef SCAPIX_JAVA_API_JAVA_UTIL_LOGGING_LOGGINGPERMISSION
#define SCAPIX_JAVA_API_JAVA_UTIL_LOGGING_LOGGINGPERMISSION

#define SCAPIX_JAVA_API_FWD
#include <scapix/java_api/java/lang/String.h>
#include <scapix/java_api/java/security/Permission.h>
#undef SCAPIX_JAVA_API_FWD

#include <scapix/jni/java_api/macros_push.h>

namespace scapix::java_api {

class java::util::logging::LoggingPermission : public jni::object_base<"java/util/logging/LoggingPermission",
	java::security::BasicPermission,
	java::security::Guard,
	java::io::Serializable>
{
public:

	static jni::ref<java::util::logging::LoggingPermission> new_object(jni::ref<java::lang::String> name, jni::ref<java::lang::String> actions) { return object_base::new_object(name, actions); }
	jni::ref<java::lang::String> getActions() { return call_method<"getActions", jni::ref<java::lang::String>>(); }
	jboolean implies(jni::ref<java::security::Permission> permission) { return call_method<"implies", jboolean>(permission); }
};

} // namespace scapix::java_api

#include <scapix/jni/java_api/macros_pop.h>

#endif // SCAPIX_JAVA_API_JAVA_UTIL_LOGGING_LOGGINGPERMISSION
#endif // SCAPIX_JAVA_API_FWD
