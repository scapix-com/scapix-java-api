// Generated by Scapix Java API 2.1
// https://www.scapix.com

#include <scapix/java_api/java/lang/Object.h>
#include <scapix/java_api/javax/swing/Action.h>

#ifndef SCAPIX_JAVA_API_SUN_SWING_UIACTION_FWD
#define SCAPIX_JAVA_API_SUN_SWING_UIACTION_FWD

#include <scapix/jni/java_api/macros_push.h>

namespace scapix::java_api { namespace sun::swing { class UIAction; }}

template<>
struct scapix::jni::object_traits<scapix::java_api::sun::swing::UIAction>
{
	static constexpr fixed_string class_name = "sun/swing/UIAction";
	using base_classes = std::tuple<scapix::java_api::java::lang::Object, scapix::java_api::javax::swing::Action>;
};

#include <scapix/jni/java_api/macros_pop.h>

#endif // SCAPIX_JAVA_API_SUN_SWING_UIACTION_FWD

#if !defined(SCAPIX_JAVA_API_FWD) && !defined(SCAPIX_JAVA_API_SUN_SWING_UIACTION)
#define SCAPIX_JAVA_API_SUN_SWING_UIACTION

#define SCAPIX_JAVA_API_FWD
#include <scapix/java_api/java/beans/PropertyChangeListener.h>
#include <scapix/java_api/java/lang/String.h>
#undef SCAPIX_JAVA_API_FWD

#include <scapix/jni/java_api/macros_push.h>

namespace scapix::java_api {

class sun::swing::UIAction : public jni::object_base<"sun/swing/UIAction",
	java::lang::Object,
	javax::swing::Action>
{
public:

	static jni::ref<sun::swing::UIAction> new_object(jni::ref<java::lang::String> p1) { return object_base::new_object(p1); }
	jni::ref<java::lang::String> getName() { return call_method<"getName", jni::ref<java::lang::String>>(); }
	jni::ref<java::lang::Object> getValue(jni::ref<java::lang::String> p1) { return call_method<"getValue", jni::ref<java::lang::Object>>(p1); }
	void putValue(jni::ref<java::lang::String> p1, jni::ref<java::lang::Object> p2) { return call_method<"putValue", void>(p1, p2); }
	void setEnabled(jboolean p1) { return call_method<"setEnabled", void>(p1); }
	jboolean isEnabled() { return call_method<"isEnabled", jboolean>(); }
	jboolean isEnabled(jni::ref<java::lang::Object> p1) { return call_method<"isEnabled", jboolean>(p1); }
	void addPropertyChangeListener(jni::ref<java::beans::PropertyChangeListener> p1) { return call_method<"addPropertyChangeListener", void>(p1); }
	void removePropertyChangeListener(jni::ref<java::beans::PropertyChangeListener> p1) { return call_method<"removePropertyChangeListener", void>(p1); }

};

} // namespace scapix::java_api

#include <scapix/jni/java_api/macros_pop.h>

#endif // SCAPIX_JAVA_API_SUN_SWING_UIACTION
