// Generated by Scapix Java API 2.3
// https://www.scapix.com

#include <scapix/java_api/java/io/Writer.h>

#ifndef SCAPIX_JAVA_API_JAVA_IO_PRINTWRITER_FWD
#define SCAPIX_JAVA_API_JAVA_IO_PRINTWRITER_FWD

#include <scapix/jni/java_api/macros_push.h>

namespace scapix::java_api {
namespace java::io {

class PrintWriter;

} // namespace java::io
} // namespace scapix::java_api

template<>
struct scapix::jni::object_traits<scapix::java_api::java::io::PrintWriter>
{
	static constexpr fixed_string class_name = "java/io/PrintWriter";
	using base_classes = std::tuple<scapix::java_api::java::io::Writer>;
};

#include <scapix/jni/java_api/macros_pop.h>

#endif // SCAPIX_JAVA_API_JAVA_IO_PRINTWRITER_FWD

#ifndef SCAPIX_JAVA_API_FWD
#ifndef SCAPIX_JAVA_API_JAVA_IO_PRINTWRITER
#define SCAPIX_JAVA_API_JAVA_IO_PRINTWRITER

#define SCAPIX_JAVA_API_FWD
#include <scapix/java_api/java/io/File.h>
#include <scapix/java_api/java/io/OutputStream.h>
#include <scapix/java_api/java/lang/CharSequence.h>
#include <scapix/java_api/java/lang/Object.h>
#include <scapix/java_api/java/lang/String.h>
#include <scapix/java_api/java/util/Locale.h>
#undef SCAPIX_JAVA_API_FWD

#include <scapix/jni/java_api/macros_push.h>

namespace scapix::java_api {

class java::io::PrintWriter : public jni::object_base<"java/io/PrintWriter",
	java::io::Writer>
{
public:

	static jni::ref<java::io::PrintWriter> new_object(jni::ref<java::io::OutputStream> out) { return object_base::new_object(out); }
	static jni::ref<java::io::PrintWriter> new_object(jni::ref<java::io::OutputStream> out, jboolean autoFlush) { return object_base::new_object(out, autoFlush); }
	static jni::ref<java::io::PrintWriter> new_object(jni::ref<java::io::Writer> wr) { return object_base::new_object(wr); }
	static jni::ref<java::io::PrintWriter> new_object(jni::ref<java::io::Writer> wr, jboolean autoFlush) { return object_base::new_object(wr, autoFlush); }
	static jni::ref<java::io::PrintWriter> new_object(jni::ref<java::io::File> file) { return object_base::new_object(file); }
	static jni::ref<java::io::PrintWriter> new_object(jni::ref<java::io::File> file, jni::ref<java::lang::String> csn) { return object_base::new_object(file, csn); }
	static jni::ref<java::io::PrintWriter> new_object(jni::ref<java::lang::String> fileName) { return object_base::new_object(fileName); }
	static jni::ref<java::io::PrintWriter> new_object(jni::ref<java::lang::String> fileName, jni::ref<java::lang::String> csn) { return object_base::new_object(fileName, csn); }
	jboolean checkError() { return call_method<"checkError", jboolean>(); }
	void close() { return call_method<"close", void>(); }
	void flush() { return call_method<"flush", void>(); }
	jni::ref<java::io::PrintWriter> format(jni::ref<java::lang::String> format, jni::ref<jni::array<java::lang::Object>> args) { return call_method<"format", jni::ref<java::io::PrintWriter>>(format, args); }
	jni::ref<java::io::PrintWriter> format(jni::ref<java::util::Locale> l, jni::ref<java::lang::String> format, jni::ref<jni::array<java::lang::Object>> args) { return call_method<"format", jni::ref<java::io::PrintWriter>>(l, format, args); }
	jni::ref<java::io::PrintWriter> printf(jni::ref<java::lang::String> format, jni::ref<jni::array<java::lang::Object>> args) { return call_method<"printf", jni::ref<java::io::PrintWriter>>(format, args); }
	jni::ref<java::io::PrintWriter> printf(jni::ref<java::util::Locale> l, jni::ref<java::lang::String> format, jni::ref<jni::array<java::lang::Object>> args) { return call_method<"printf", jni::ref<java::io::PrintWriter>>(l, format, args); }
	void print(jni::ref<jni::array<jchar>> charArray) { return call_method<"print", void>(charArray); }
	void print(jchar ch) { return call_method<"print", void>(ch); }
	void print(jdouble dnum) { return call_method<"print", void>(dnum); }
	void print(jfloat fnum) { return call_method<"print", void>(fnum); }
	void print(jint inum) { return call_method<"print", void>(inum); }
	void print(jlong lnum) { return call_method<"print", void>(lnum); }
	void print(jni::ref<java::lang::Object> obj) { return call_method<"print", void>(obj); }
	void print(jni::ref<java::lang::String> str) { return call_method<"print", void>(str); }
	void print(jboolean bool_) { return call_method<"print", void>(bool_); }
	void println() { return call_method<"println", void>(); }
	void println(jni::ref<jni::array<jchar>> chars) { return call_method<"println", void>(chars); }
	void println(jchar c) { return call_method<"println", void>(c); }
	void println(jdouble d) { return call_method<"println", void>(d); }
	void println(jfloat f) { return call_method<"println", void>(f); }
	void println(jint i) { return call_method<"println", void>(i); }
	void println(jlong l) { return call_method<"println", void>(l); }
	void println(jni::ref<java::lang::Object> obj) { return call_method<"println", void>(obj); }
	void println(jni::ref<java::lang::String> str) { return call_method<"println", void>(str); }
	void println(jboolean b) { return call_method<"println", void>(b); }
	void write(jni::ref<jni::array<jchar>> buf) { return call_method<"write", void>(buf); }
	void write(jni::ref<jni::array<jchar>> buf, jint offset, jint count) { return call_method<"write", void>(buf, offset, count); }
	void write(jint oneChar) { return call_method<"write", void>(oneChar); }
	void write(jni::ref<java::lang::String> str) { return call_method<"write", void>(str); }
	void write(jni::ref<java::lang::String> str, jint offset, jint count) { return call_method<"write", void>(str, offset, count); }
	jni::ref<java::io::PrintWriter> append(jchar c) { return call_method<"append", jni::ref<java::io::PrintWriter>>(c); }
	jni::ref<java::io::PrintWriter> append(jni::ref<java::lang::CharSequence> csq) { return call_method<"append", jni::ref<java::io::PrintWriter>>(csq); }
	jni::ref<java::io::PrintWriter> append(jni::ref<java::lang::CharSequence> csq, jint start, jint end) { return call_method<"append", jni::ref<java::io::PrintWriter>>(csq, start, end); }
};

} // namespace scapix::java_api

#include <scapix/jni/java_api/macros_pop.h>

#endif // SCAPIX_JAVA_API_JAVA_IO_PRINTWRITER
#endif // SCAPIX_JAVA_API_FWD
