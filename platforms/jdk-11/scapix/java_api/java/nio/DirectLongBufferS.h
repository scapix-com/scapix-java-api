// Generated by Scapix Java API 2.1
// https://www.scapix.com

#include <scapix/java_api/java/nio/LongBuffer.h>
#include <scapix/java_api/sun/nio/ch/DirectBuffer.h>

#ifndef SCAPIX_JAVA_API_JAVA_NIO_DIRECTLONGBUFFERS_FWD
#define SCAPIX_JAVA_API_JAVA_NIO_DIRECTLONGBUFFERS_FWD

#include <scapix/jni/java_api/macros_push.h>

namespace scapix::java_api { namespace java::nio { class DirectLongBufferS; }}

template<>
struct scapix::jni::object_traits<scapix::java_api::java::nio::DirectLongBufferS>
{
	static constexpr fixed_string class_name = "java/nio/DirectLongBufferS";
	using base_classes = std::tuple<scapix::java_api::java::nio::LongBuffer, scapix::java_api::sun::nio::ch::DirectBuffer>;
};

#include <scapix/jni/java_api/macros_pop.h>

#endif // SCAPIX_JAVA_API_JAVA_NIO_DIRECTLONGBUFFERS_FWD

#if !defined(SCAPIX_JAVA_API_FWD) && !defined(SCAPIX_JAVA_API_JAVA_NIO_DIRECTLONGBUFFERS)
#define SCAPIX_JAVA_API_JAVA_NIO_DIRECTLONGBUFFERS

#define SCAPIX_JAVA_API_FWD
#include <scapix/java_api/java/lang/Object.h>
#include <scapix/java_api/java/nio/ByteOrder.h>
#include <scapix/java_api/jdk/internal/ref/Cleaner.h>
#undef SCAPIX_JAVA_API_FWD

#include <scapix/jni/java_api/macros_push.h>

namespace scapix::java_api {

// private
class java::nio::DirectLongBufferS : public jni::object_base<"java/nio/DirectLongBufferS",
	java::nio::LongBuffer,
	sun::nio::ch::DirectBuffer>
{
public:

	jni::ref<java::lang::Object> attachment() { return call_method<"attachment", jni::ref<java::lang::Object>>(); }
	jni::ref<jdk::internal::ref::Cleaner> cleaner() { return call_method<"cleaner", jni::ref<jdk::internal::ref::Cleaner>>(); }
	jni::ref<java::nio::LongBuffer> slice() { return call_method<"slice", jni::ref<java::nio::LongBuffer>>(); }
	jni::ref<java::nio::LongBuffer> duplicate() { return call_method<"duplicate", jni::ref<java::nio::LongBuffer>>(); }
	jni::ref<java::nio::LongBuffer> asReadOnlyBuffer() { return call_method<"asReadOnlyBuffer", jni::ref<java::nio::LongBuffer>>(); }
	jlong address() { return call_method<"address", jlong>(); }
	jlong get() { return call_method<"get", jlong>(); }
	jlong get(jint i) { return call_method<"get", jlong>(i); }
	jni::ref<java::nio::LongBuffer> get(jni::ref<jni::array<jlong>> dst, jint offset, jint length) { return call_method<"get", jni::ref<java::nio::LongBuffer>>(dst, offset, length); }
	jni::ref<java::nio::LongBuffer> put(jlong x) { return call_method<"put", jni::ref<java::nio::LongBuffer>>(x); }
	jni::ref<java::nio::LongBuffer> put(jint i, jlong x) { return call_method<"put", jni::ref<java::nio::LongBuffer>>(i, x); }
	jni::ref<java::nio::LongBuffer> put(jni::ref<java::nio::LongBuffer> src) { return call_method<"put", jni::ref<java::nio::LongBuffer>>(src); }
	jni::ref<java::nio::LongBuffer> put(jni::ref<jni::array<jlong>> src, jint offset, jint length) { return call_method<"put", jni::ref<java::nio::LongBuffer>>(src, offset, length); }
	jni::ref<java::nio::LongBuffer> compact() { return call_method<"compact", jni::ref<java::nio::LongBuffer>>(); }
	jboolean isDirect() { return call_method<"isDirect", jboolean>(); }
	jboolean isReadOnly() { return call_method<"isReadOnly", jboolean>(); }
	jni::ref<java::nio::ByteOrder> order() { return call_method<"order", jni::ref<java::nio::ByteOrder>>(); }

};

} // namespace scapix::java_api

#include <scapix/jni/java_api/macros_pop.h>

#endif // SCAPIX_JAVA_API_JAVA_NIO_DIRECTLONGBUFFERS
