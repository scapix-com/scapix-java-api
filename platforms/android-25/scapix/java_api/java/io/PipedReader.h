// Generated by Scapix Java API 2.3
// https://www.scapix.com

#include <scapix/java_api/java/io/Reader.h>

#ifndef SCAPIX_JAVA_API_JAVA_IO_PIPEDREADER_FWD
#define SCAPIX_JAVA_API_JAVA_IO_PIPEDREADER_FWD

#include <scapix/jni/java_api/macros_push.h>

namespace scapix::java_api {
namespace java::io {

class PipedReader;

} // namespace java::io
} // namespace scapix::java_api

template<>
struct scapix::jni::object_traits<scapix::java_api::java::io::PipedReader>
{
	static constexpr fixed_string class_name = "java/io/PipedReader";
	using base_classes = std::tuple<scapix::java_api::java::io::Reader>;
};

#include <scapix/jni/java_api/macros_pop.h>

#endif // SCAPIX_JAVA_API_JAVA_IO_PIPEDREADER_FWD

#ifndef SCAPIX_JAVA_API_FWD
#ifndef SCAPIX_JAVA_API_JAVA_IO_PIPEDREADER
#define SCAPIX_JAVA_API_JAVA_IO_PIPEDREADER

#define SCAPIX_JAVA_API_FWD
#include <scapix/java_api/java/io/PipedWriter.h>
#undef SCAPIX_JAVA_API_FWD

#include <scapix/jni/java_api/macros_push.h>

namespace scapix::java_api {

class java::io::PipedReader : public jni::object_base<"java/io/PipedReader",
	java::io::Reader>
{
public:

	static jni::ref<java::io::PipedReader> new_object(jni::ref<java::io::PipedWriter> src) { return object_base::new_object(src); }
	static jni::ref<java::io::PipedReader> new_object(jni::ref<java::io::PipedWriter> src, jint pipeSize) { return object_base::new_object(src, pipeSize); }
	static jni::ref<java::io::PipedReader> new_object() { return object_base::new_object(); }
	static jni::ref<java::io::PipedReader> new_object(jint pipeSize) { return object_base::new_object(pipeSize); }
	void connect(jni::ref<java::io::PipedWriter> src) { return call_method<"connect", void>(src); }
	jint read() { return call_method<"read", jint>(); }
	jint read(jni::ref<jni::array<jchar>> cbuf, jint off, jint len) { return call_method<"read", jint>(cbuf, off, len); }
	jboolean ready() { return call_method<"ready", jboolean>(); }
	void close() { return call_method<"close", void>(); }
};

} // namespace scapix::java_api

#include <scapix/jni/java_api/macros_pop.h>

#endif // SCAPIX_JAVA_API_JAVA_IO_PIPEDREADER
#endif // SCAPIX_JAVA_API_FWD
